...
class CustomReflexAgent(Agent):
    def getAction(self, gameState):
        legalMoves = gameState.getLegalActions()

        # Choose one of the legal moves based on a simple evaluation function
        scores = [self.evaluationFunction(gameState, action) for action in legalMoves]
        bestScore = max(scores)
        bestIndices = [index for index in range(len(scores)) if scores[index] == bestScore]
    ...
    def evaluationFunction(self, currentGameState, action):
        # Simple evaluation function: maximize the distance to the nearest ghost
        successorGameState = currentGameState.generatePacmanSuccessor(action)
        ghostPositions = successorGameState.getGhostPositions()
        pacmanPosition = successorGameState.getPacmanPosition()

        minGhostDistance = min(util.manhattanDistance(pacmanPosition, ghost) for ghost in ghostPositions)

        return -minGhostDistance  # Negative because we want to maximize the distance


class BetterReflexAgent(Agent):
    def getAction(self, gameState):
        legalMoves = gameState.getLegalActions()

        # Choose one of the legal moves based on a better evaluation function
        scores = [self.evaluationFunction(gameState, action) for action in legalMoves]
        bestScore = max(scores)
        bestIndices = [index for index in range(len(scores)) if scores[index] == bestScore]
        chosenIndex = random.choice(bestIndices)
    ...
    def evaluationFunction(self, currentGameState, action):
        ...
        minFoodDistance = min(util.manhattanDistance(pacmanPosition, food) for food in foodPositions)

        # Weighted combination of distances, prioritizing food proximity
        score = 2 * minFoodDistance - minGhostDistance

        return score
class QLearningAgent(MultiAgentSearchAgent):
    def __init__(self, evalFn='scoreEvaluationFunction', depth=2, learning_rate=0.1, discount_factor=0.9, exploration_rate=0.1):
    ...
    def getAction(self, gameState):
        ...
        if random.uniform(0, 1) < self.exploration_rate:
            ...
            return max(legalMoves, key=lambda a: self.getQValue(gameState, a))
    ...
    def updateQValue(self, state, action, reward, next_state):
        ...
        max_future_q_value = max([self.getQValue(next_state, a) for a in gameState.getLegalActions(0)])
    ...
    def expectimaxValue(self, gameState, agentIndex, depth):
        ...
        if agentIndex == 0:  # Pacman's turn (MAX)
            ...
            return max(self.expectimaxValue(gameState.generateSuccessor(agentIndex, action), 1, depth) for action in legalMoves)
        else:  # Ghost's turn (EXPECTATION)
            legalMoves = gameState.getLegalActions(agentIndex)
            numLegalMoves = len(legalMoves)
            return sum(self.expectimaxValue(gameState.generateSuccessor(agentIndex, action), (agentIndex + 1) % numAgents, depth + 1) for action in legalMoves) / numLegalMoves
